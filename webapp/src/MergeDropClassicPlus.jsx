...
function processVerticalMerges(grid) {
  ...
  for (let c = 0; c < COLS; c++) {
    for (let r = ROWS - 2; r >= 0; r--) {
      ...
      // Classic+ mode: only INCREASING merges (no decrease, no 1+1=2)
      if (top > 0 && bot > 0 && top > bot && (top - bot) === 1) {
...
export default function MergeDropClassicPlus() {
  ...
  useEffect(() => {
    getGameStats("classic_plus").then(stats => {
  ...
  useEffect(() => {
    ...
    saveGameResult("classic_plus", score, turn, duration).then(result => {
      if (result) {
