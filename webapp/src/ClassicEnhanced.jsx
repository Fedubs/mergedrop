...
function processVerticalMerges(grid) {
  ...
  for (let c = 0; c < COLS; c++) {
    for (let r = ROWS - 2; r >= 0; r--) {
      ...
      // Classic mode: only merge if numbers differ by exactly 1 (no 1+1=2)
...
function getSettings() {
  const saved = localStorage.getItem('classic_enhanced_settings');
  return saved ? JSON.parse(saved) : { audioLevel: 'normal' };
}

function saveSettings(settings) {
  localStorage.setItem('classic_enhanced_settings', JSON.stringify(settings));
}

export default function ClassicEnhanced() {
  ...
  useEffect(() => {
    getGameStats("classic_enhanced").then(stats => {
  ...
  useEffect(() => {
    ...
    saveGameResult("classic_enhanced", score, turn, duration).then(result => {
      if (result) {
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontSize: 13, color: "#997777", lineHeight: 2 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                ...
                <span><b style={{ color: "#CC9999" }}>Up merge:</b> Sequential pair going up → increases! 1+2=3, 2+3=4, 3+4=5</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>↓</span>
                <span><b style={{ color: "#CC9999" }}>Down merge:</b> Sequential pair going down → decreases! 2 on 3=1</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>↔</span>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                ...
                <span><b style={{ color: "#CC9999" }}>6</b> = explosion! Block is destroyed</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>🎯</span>
