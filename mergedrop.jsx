...
// 1+2=3, 2+3=5, 3+5=8, 5+8=13 (explode!)
...
function playTone(freq, duration, vol = 0.3, type = "sine") {
  try {
...
const haptic = {
  drop: () => { vib(15); playTone(220, 0.06, 0.15, "square"); },
  merge: (combo = 1) => {
    const pitch = 1 + (combo - 1) * 0.15;
    vib(30);
    playTone(440 * pitch, 0.1, 0.2, "sine");
    playTone(660 * pitch, 0.08, 0.15, "sine");
  },
  chain: (combo = 1) => {
    const pitch = 1 + (combo - 1) * 0.18;
    vib([20, 30, 40]);
    playTone(523 * pitch, 0.08, 0.2, "sine");
    setTimeout(() => playTone(659 * pitch, 0.08, 0.2, "sine"), 60);
    setTimeout(() => playTone(784 * pitch, 0.12, 0.25, "sine"), 120);
  },
  explode: (combo = 1) => {
    vib([40, 20, 60, 20, 80]);
    ...
    try {
      ...
      pop.type = "sine";
      pop.frequency.setValueAtTime(180 * pitch, t);
      pop.frequency.exponentialRampToValueAtTime(40 * pitch, t + 0.15);
      ...
      // Watery splash — filtered noise
      const splashLen = 0.35;
      const buf = ctx.createBuffer(1, ctx.sampleRate * splashLen, ctx.sampleRate);
      const d = buf.getChannelData(0);
      for (let i = 0; i < d.length; i++) {
        const env = Math.pow(1 - i / d.length, 2);
        d[i] = (Math.random() * 2 - 1) * env * (0.5 + 0.5 * Math.sin(i * 0.05));
      }
      const src = ctx.createBufferSource();
      src.buffer = buf;
      const filter = ctx.createBiquadFilter();
      filter.type = "bandpass";
      filter.frequency.setValueAtTime(2500 * pitch, t);
      filter.frequency.exponentialRampToValueAtTime(600 * pitch, t + splashLen);
      filter.Q.value = 1.5;
      const splashGain = ctx.createGain();
      splashGain.gain.setValueAtTime(0.3, t + 0.02);
      splashGain.gain.exponentialRampToValueAtTime(0.001, t + splashLen);
      src.connect(filter);
      ...
      bub.type = "sine";
      bub.frequency.setValueAtTime(350 * pitch, t + 0.03);
      bub.frequency.exponentialRampToValueAtTime(80 * pitch, t + 0.25);
      const lfo = ctx.createOscillator();
      const lfoGain = ctx.createGain();
      lfo.frequency.value = 25;
      lfoGain.gain.value = 60;
      lfo.connect(lfoGain);
      lfoGain.connect(bub.frequency);
      lfo.start(t + 0.03);
      lfo.stop(t + 0.3);
      bubGain.gain.setValueAtTime(0.2, t + 0.03);
      bubGain.gain.exponentialRampToValueAtTime(0.001, t + 0.28);
      bub.connect(bubGain);
      bubGain.connect(ctx.destination);
      bub.start(t + 0.03);
      bub.stop(t + 0.3);
    } catch(e) {}
  },
  rise: () => { vib([15, 10, 15, 10, 15]); playTone(150, 0.08, 0.12, "square"); setTimeout(() => playTone(180, 0.08, 0.12, "square"), 80); },
  gameOver: () => { vib([100, 50, 100, 50, 200]); playTone(300, 0.15, 0.25, "sawtooth"); setTimeout(() => playTone(200, 0.15, 0.25, "sawtooth"), 150); setTimeout(() => playTone(100, 0.4, 0.3, "sawtooth"), 300); },
};
...
function randomShade(turn = 0) {
  const maxShade = Math.min(4 + Math.floor(turn / 10), 5);
  const weights = [];
  for (let i = 1; i <= maxShade; i++) weights.push({ shade: i, w: maxShade - i + 3 });
...
function processVerticalMerges(grid) {
  ...
  for (let c = 0; c < COLS; c++) {
    for (let r = ROWS - 2; r >= 0; r--) {
      ...
      if (top > 0 && bot > 0 && (Math.abs(top - bot) === 1 || (top === 1 && bot === 1))) {
        if (top === 1 && bot === 1) {
          // 1+1=2 → result goes UP
          consumed.push({ r: r + 1, c, targetR: r, targetC: c, shade: bot });
          grid[r + 1][c] = 0;
          grid[r][c] = 2;
          merges.push({ r, c, newShade: 2 });
        } else if (top > bot) {
          // Increasing upward → result goes UP
          ...
          // Decreasing upward (top < bot) → result goes DOWN
          const newShade = Math.max(top - 1, 1);
...
export default function MergeDropGame() {
  ...
  const pushRowUp = useCallback((g, turnNum) => {
    ...
    // Fill bottom row with random shades
    for (let c = 0; c < COLS; c++) {
      g[ROWS - 1][c] = randomShade(turnNum);
    }
        ...
        if (chainResult.steps.length > 0) {
          ...
          const playRisingChain = () => {
            if (stepIdx >= chainResult.steps.length) {
              ...
              if (isOver) { setGameOver(true); haptic.gameOver(); }
              setScore(prev => {
  ...
  return (
    <div ref={gameRef} tabIndex={0} style={{
      ...
      {/* Help & Restart buttons */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginTop: 20, marginBottom: 12 }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 9, color: "#553333", letterSpacing: 2, marginTop: 3 }}>TURN {turn}</div>
      ...
      <div style={{ marginTop: "auto", marginBottom: 60, width: "100%" }}>
      {/* Column arrows */}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {grid.map((row, r) =>
            row.map((cell, c) => {
              ...
              else if (cell === 6) animClass = "cell-shade6";
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: cell === 6 ? Math.round(CELL_SIZE * 0.45) : cell > 0 ? Math.round(CELL_SIZE * 0.35) : 0, fontWeight: 800,
                    ...
                    zIndex: isConsumed ? 5 : isMerge ? 4 : cell === 6 ? 3 : 2,
                  }}
                >
                  {cell > 0 && (
                    <>
        ...
        {gameOver && (
          <div style={{
            ...
            <div style={{ fontSize: 14, color: "#886666", marginBottom: 24 }}>Turns: {turn}</div>
            <button onClick={resetGame} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontSize: 13, color: "#997777", lineHeight: 2 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                ...
                <span><b style={{ color: "#CC9999" }}>Sum up:</b> Fibonacci pair going up → sum rises! 2+3=5, 3+5=8</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>↓</span>
                <span><b style={{ color: "#CC9999" }}>Diff down:</b> Fibonacci pair going down → difference sinks! 2 on 3=1</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>↔</span>
                <span><b style={{ color: "#CC9999" }}>Horizontal:</b> 3+ same number in a row merge</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#FF4444", fontWeight: 700, minWidth: 16 }}>13</span>
                <span><b style={{ color: "#CC9999" }}>13</b> = explosion! Block is destroyed</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 6 }}>
                <span style={{ color: "#CC6666", fontWeight: 700, minWidth: 16 }}>🎯</span>
                <span><b style={{ color: "#CC9999" }}>Chain</b> reactions multiply your score!</span>
              </div>
